...
export class Migrations1719608322294 implements MigrationInterface {
    name = 'Migrations1719608322294'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`product\` (\`Id\` int NOT NULL AUTO_INCREMENT, \`Uuid\` varchar(36) NULL, \`Name\` varchar(255) NOT NULL, \`Price\` decimal NOT NULL, \`TotalItens\` int NOT NULL, \`BarCode\` varchar(255) NULL, \`Column\` varchar(255) NULL, \`Shelf\` varchar(255) NULL, \`stockId\` int NULL, UNIQUE INDEX \`IDX_a7207424fa5d74c694b2249bd4\` (\`Uuid\`), PRIMARY KEY (\`Id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`stock\` (\`Id\` int NOT NULL AUTO_INCREMENT, \`Uuid\` varchar(36) NULL, UNIQUE INDEX \`IDX_68d9da1b2b1a76db234791e39f\` (\`Uuid\`), PRIMARY KEY (\`Id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`order_item\` (\`Id\` int NOT NULL AUTO_INCREMENT, \`Uuid\` varchar(36) NULL, \`Name\` varchar(255) NOT NULL, \`Price\` decimal NOT NULL, \`Quantity\` int NOT NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`orderIdId\` int NULL, UNIQUE INDEX \`IDX_48c49fd17d80ab1ec799b11c23\` (\`Uuid\`), PRIMARY KEY (\`Id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`order\` (\`Id\` int NOT NULL AUTO_INCREMENT, \`Uuid\` varchar(36) NULL, \`OrderType\` varchar(255) NOT NULL, \`DeliveryAddress\` json NULL, \`Status\` varchar(255) NOT NULL, \`Observations\` varchar(255) NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`tenantId\` int NULL, UNIQUE INDEX \`IDX_bb2c0a3f67db7a656b1a1d4d02\` (\`Uuid\`), PRIMARY KEY (\`Id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`tenant\` (\`Id\` int NOT NULL AUTO_INCREMENT, \`Uuid\` varchar(36) NULL, \`Name\` varchar(255) NOT NULL, \`IsActive\` tinyint NOT NULL DEFAULT 1, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`stockId\` int NULL, UNIQUE INDEX \`IDX_880c9526ab0b79e3b147e4d0c5\` (\`Uuid\`), UNIQUE INDEX \`REL_3176ac89b41d2af6731cb08eac\` (\`stockId\`), PRIMARY KEY (\`Id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`administrator\` (\`Id\` int NOT NULL AUTO_INCREMENT, \`Uuid\` varchar(36) NULL, \`IsActive\` tinyint NULL DEFAULT 1, \`EmailAddr\` varchar(255) NOT NULL, \`HashPassword\` varchar(255) NOT NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_b7d13040b5ba815ca3dc268e23\` (\`Uuid\`), UNIQUE INDEX \`IDX_16cf9bed99d7a97d554e4b4e19\` (\`EmailAddr\`), PRIMARY KEY (\`Id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`IDX_d95d3efeb42690697eed2e54fa\` ON \`person\``);
