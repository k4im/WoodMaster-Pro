...
@ApiBearerAuth()
export default class ReactiveCollaboratorController {
    constructor(
        @Inject('reactivateCollab')
        private readonly reactivateCollabUseCase: ISingleCommandInterface<boolean>,
    ) { }

    @Post('collaborator/:tenantId/reactivate/:uuid')
    ...
    @ApiOperation({
        summary: 'Rota utilizada para reativar colaborador.',
        description: `Podera ser utilizado para realizar a operação
        de reativação de um colaborador. Portanto caso a operação seja realizada o
    ...
    async handle(@Param() { uuid, tenantId }: any, @Res() res: Response) {
        const result = await this.reactivateCollabUseCase.execute(uuid, tenantId)
        result ?
            res.status(200).send({ message: 'collaborator reactivated.' }) :
